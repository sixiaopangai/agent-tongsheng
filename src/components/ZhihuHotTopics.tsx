            ...
            {topic.id && (
              <button
                ...
                className={`shrink-0 p-1.5 rounded-md hover:bg-slate-700 transition ${
                  likedIds.has(topic.id) ? 'text-orange-400' : 'text-slate-500'
                }`}
                title={likedIds.has(topic.id) ? '取消点赞' : '点赞'}
              >
                <svg className="w-4 h-4" fill={likedIds.has(topic.id) ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
